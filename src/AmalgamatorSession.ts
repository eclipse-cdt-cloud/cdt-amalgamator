...
import {
    InitializedEvent, Logger, logger, LoggingDebugSession, Event, Handles,
...
export class AmalgamatorSession extends LoggingDebugSession {
    ...
    protected async launchRequest(response: DebugProtocol.LaunchResponse, args: LaunchRequestArguments): Promise<void> {
        ...
        for (const child of args.children) {
            if (child.delay) {
                logger.verbose(`waiting ${child.delay}ms before starting ${child.debugAdapterRuntime} ${child.debugAdapterExecutable}`);
    ...
    protected async createChild(child: ChildDapArguments, index: number) {
        logger.verbose(`creating debug adapter ${child.debugAdapterRuntime} ${child.debugAdapterExecutable}`);
        const dc = new DebugClient(child.debugAdapterRuntime, child.debugAdapterExecutable, 'unused');
        ...
        dc.on('output', event => {
            ...
            if (e.body.category == 'stdout') {
                ...
                if (output.startsWith('To client: ')) {
                    output = 'To amalgamator: ' + output.substr('To client: '.length);
                    logger.verbose(output);
                } else if (output.startsWith('From client: ')) {
                    output = 'From amalgamator: ' + output.substr('From client: '.length);
                    logger.verbose(output);
                }
                this.sendEvent(e);
            } else {
            ...
            if (e.body.allThreadsStopped) {
                threadMap.forEach(([childDapIndex, childId], clientId) => {
                    if (childDapIndex === stoppedDapIndex && childId != intiatingThreadId) {
    ...
    protected async setBreakPointsRequest(response: DebugProtocol.SetBreakpointsResponse, args: DebugProtocol.SetBreakpointsArguments): Promise<void> {
        const responses = await Promise.all(this.childDaps.map(dc => dc.setBreakpointsRequest(args)));
            ...
            for (let i = 0; i < args.breakpoints.length; i++) {
                responses.forEach((response, i) => {
                    response.body.breakpoints.forEach(bp => {
                        if (bp.id) {
                            bp.id = this.breakpointHandles.create([this.childDaps[i], bp.id]);
                        }
                    });
                });
    ...
    protected async configurationDoneRequest(response: DebugProtocol.ConfigurationDoneResponse, args: DebugProtocol.ConfigurationDoneArguments): Promise<void> {
        await Promise.all(this.childDaps.map(async (dc) => await dc.configurationDoneRequest(args)));
     ...
     */
    protected async getThreadInfo(clientThreadId: number): Promise<[number, number]> {
        const threadMap = await this.getThreadMap();
    ...
    private threadMapInProcess: (Promise<[Map<number, [number, number]>, DebugProtocol.Thread[]]> | undefined);
    protected async getThreadMap(): Promise<Map<number, [number, number]>> {
        return new Promise<Map<number, [number, number]>>((resolve, reject) => {
            this.getThreadMapInternal().then(([threadMap,]) => resolve(threadMap));
        });
    }
    protected getThreadMapInternal(): Promise<[Map<number, [number, number]>, DebugProtocol.Thread[]]> {
    ...
    private collectChildTheads(): Promise<[Map<number, [number, number]>, DebugProtocol.Thread[]]> {
        this.threadMapInProcess = new Promise(async (resolve, reject) => {
            let threads: DebugProtocol.Thread[] = [];
            const responses = await Promise.all(this.childDaps.map(dc => dc.threadsRequest()));
            const threadMap: Map<number, [number, number]> = new Map<number, [number, number]>()
            let clientId = 1000;
    ...
    protected async stackTraceRequest(response: DebugProtocol.StackTraceResponse, args: DebugProtocol.StackTraceArguments): Promise<void> {
        ...
        frames.forEach(frame => frame.id = this.frameHandles.create([childDap, frame.id]));
    ...
    protected async scopesRequest(response: DebugProtocol.ScopesResponse, args: DebugProtocol.ScopesArguments): Promise<void> {
        ...
        scopes.body.scopes.forEach(scope => scope.variablesReference = this.variableHandles.create([childDap, scope.variablesReference]));
    ...
    protected async variablesRequest(response: DebugProtocol.VariablesResponse, args: DebugProtocol.VariablesArguments): Promise<void> {
        ...
        variables.body.variables.forEach(variable => {
            if (variable.variablesReference) {
                variable.variablesReference = this.variableHandles.create([childDap, variable.variablesReference])
            }
        });
    ...
    protected async customRequest(command: string, response: DebugProtocol.Response, args: any): Promise<void> {
        ...
        const childResponse = await this.childDaps[0].customRequest(command, args);
        response.body = childResponse.body;
        this.sendResponse(response);
    }
